...
- 关键类：
  ...
  - `LeaderAgent extends BaseAgent`：重写 `run`，专注于 `analyzeTask -> validateDAG -> dispatch`。
    - 调用 CapRegistry.getActiveAgents() 获取实时资源池。
    - 构建包含资源池信息的 Prompt。
    - 调用 LLM 进行单次任务分析，同时返回意图分析、subtasks 数组和 confidenceScore。
    - 若 confidenceScore < 0.7 或 subtasks 为空，返回具体的 clarificationQuestions 要求用户补充信息。
...
- 职责：对应 F02/F03，集中管理模型、Agent Prompt 配置（角色定位、行为规范、能力边界）、Skill/工具及其权限策略，为 `AgentRuntime` 和 `TaskOrchestrator` 提供统一查询接口。
- 关键类：
  - `ModelHub`：
    - `registerModel(config: ModelConfig): Promise<ModelConfigRecord>`：注册模型到 `model_configs` 表。
    - `testConnection(id: string): Promise<ModelHealth>`：在 10 秒内完成连通性测试，返回健康状态和错误信息。
    - `getModel(id: string): Promise<ModelConfigRecord | null>`：根据 ID 获取模型配置。
    - `getModels(): Promise<ModelConfigRecord[]>`：获取所有模型列表。
    - `updateModel(id: string, updates: Partial<ModelConfig>): Promise<void>`：更新模型配置。
    - `deleteModel(id: string): Promise<void>`：删除模型配置。
    - `buildModelConfig(record: ModelConfigRecord): ModelConfig`：将数据库记录转换为运行时配置对象。
    - `getProvider(config: ModelConfig): ILlmProvider`：根据模型配置返回对应的 LLM Provider 实例。
  - `ModelConfig`（运行时配置接口）：
    ```ts
  ...
  - `ILlmProvider`（模型提供商接口）：
    ...
    ```
  - **统一调用流程**：
    1. Agent 通过 `model_config_id` 外键关联 `model_configs` 表
    2. LeaderAgent/DomainAgent 执行任务时，从 `model_configs` 表读取配置
    3. 调用 `ModelHub.buildModelConfig()` 转换为运行时配置
    4. 调用 `ModelHub.getProvider()` 获取对应的 Provider 实例
    5. 通过统一的 `ILlmProvider` 接口进行模型调用，屏蔽底层差异
  - **提供商适配**：
    - OpenAI：使用官方 `openai` SDK
    - OpenRouter：使用官方 SDK，baseURL 指向 OpenRouter 端点
  - `SkillRegistry`：
...
- 主要方法：
  ...
  - `getAllowedDomains(agentId: UUID): string[]`：获取 HTTP 白名单（供配置/展示；具体以各工具实现为准）
  - 内置基础工具集（F03.3 实现）
  | 工具名 | 功能描述 | 权限粒度 | 默认策略 |
  |--------|----------|----------|----------|
  ...
  | `exec_shell` | 执行 Shell 命令 | 策略裁决 + 参数级危险字符过滤 | DENY |
  | `http_request` | 发送 HTTP 请求 | 域名白名单/方法限制 | ASK |
  | `system_info` | 获取系统信息 | ALLOW |
  | `load_more` | 加载 skill 完整内容(SKILL.md)或 references 文档 | - | ALLOW |

  - Skill Lazy Load 设计（与 `AgentRuntime` 实现对齐）：
    - **系统 prompt**：仅列出当前 Agent 已绑定 skill 的**名称与简短描述**，不嵌入完整 `SKILL.md`。
    - **`load_more` 为解锁条件**：模型须先调用 `load_more` 读取**主文档**（根目录 `SKILL.md`，即**不传** `reference` 参数）且调用成功；此后该 skill 才视为「已加载」。
    - **动态工具列表（LLM `tools`）**：每次请求模型时，下发给 Chat Completions 的 function 列表为——全部**非** `skill:*` 内置工具（含 `load_more`） + 仅属于**已绑定**且**本任务已 `load_more` 主 SKILL.md 成功**的 skill 的 `skill:{skillName}:{toolName}`。未解锁的 skill 工具**不会出现在 API schema 中**。
    - **执行硬门禁**：即使通过其它方式构造对 `skill:*` 的调用，`AgentRuntime` 在调用 `ToolHub` 之前仍校验：对应 skill **name** 必须已在本任务中完成上述主 `SKILL.md` 的 `load_more`，否则直接返回错误，**不执行** skill 脚本。
    - **`reference` 仅补充文档**：`load_more(skill_name=..., reference=...)` 只读取 `{skillRootDir}/references/{reference}.md`，**不会**单独解锁 skill 工具；解锁仍以**成功加载主 `SKILL.md`**为准。
    - `load_more` 参数：
      - `skill_name`（必填）：要加载的 skill 名称（与 `skill:{skillName}:*` 中的 **name** 一致）
      - `reference`（可选）：要加载的 references 文档名称（不含 .md 后缀），需配合 skill_name 使用
    - 加载 references 文档路径：`{skillRootDir}/references/{reference}.md`

#### 2.2.9 工具中枢（`ToolHub`）
- 职责：统一注册并执行**系统内置**工具与 **Skill 工具**（`SkillToolImpl`，名称为 `skill:{skillName}:{toolName}`），但二者语义不同：
  - **内置工具**：在 `ToolHub` 构造时注册，所有 Agent 默认可用同一套；`listBuiltinTools()` / Agent 配置里的 `tools` 仅指这一类。
  - **Skill 工具**：随 Skill 安装注册进同一 `Map` 便于 `execute` 路由，但**不**算“系统默认工具”；`AgentRuntime` 仅在对应 skill 已绑定且本任务已成功 `load_more` 主 `SKILL.md` 后，才把该 skill 的 descriptors 合并进发给大模型的 `tools` 列表（见 2.2.8）。
  - 内置与 Skill 工具均由 `ToolHub.execute` 统一执行，并走同一套权限与参数校验。
- 关键类：
  - `BaseTool`（抽象类）：
    - name: string：工具唯一标识
...
- `HttpTool extends BaseTool`：
  ...
  - 禁止 POST/PUT 敏感端点
  - 限制请求大小和超时
- `ShellTool extends BaseTool`（`exec_shell`）：
  - 是否可执行由 `agent_tool_permissions` 等策略（默认多为 `DENY`）决定。
  - 在允许执行的路径上，对 `command` 字符串做危险字符/模式过滤（如管道、重定向、分号等），并在工作空间目录下执行、带超时。
...
CREATE TABLE model_configs (
  ...
  temperature REAL,
  status TEXT DEFAULT 'offline' CHECK(status IN ('ready', 'loading', 'error', 'offline')),
...
**温度优先级**：`agents.temperature` > `model_configs.temperature`
...
CREATE TABLE agents (
  ...
  temperature REAL,
  status TEXT DEFAULT 'IDLE' CHECK(status IN ('IDLE', 'RUNNING', 'BUSY_WITH_QUEUE', 'OFFLINE')),
  role_prompt TEXT,
  behavior_rules TEXT,
  capability_boundary TEXT,
  last_capability_heartbeat DATETIME,
  capability_status TEXT DEFAULT 'OFFLINE' CHECK(capability_status IN ('ONLINE', 'BUSY', 'OFFLINE')),
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_agents_status ON agents(status);
CREATE INDEX idx_agents_model_config ON agents(model_config_id);
CREATE INDEX idx_agents_capability_status ON agents(capability_status);
```

注：`agents.status` 表示任务队列 / 运行宏观状态；`capability_status` 与 `last_capability_heartbeat` 表示可调度性视角的能力在线状态。Agent 已绑定的 **Skill 名称列表** 不冗余落库，仅通过 `agent_skills` 与 `skills` 关联查询得到（不再使用 `skills_json`）。**系统内置工具**由 `ToolHub` 在代码中统一注册，所有 Agent 默认可用同一套内置工具名列表，不在 `agents` 表存储 `tools_json`。

#### 3.1.2 `skills`（本地 Skill 包）表
```sql
CREATE TABLE skills (
...
#### 3.1.3 `agent_skills`（Agent-Skill 绑定关系）表
```sql
...
#### 3.1.4 `agent_tool_permissions`（基础工具权限配置）表
```sql
  ...
  async run(task: Task): Promise<void> {
    try {
      ...
      const contextResources = JSON.stringify(availableAgents.map(a => ({
        ...
        capabilityBoundary: a.behaviorRules
      })));
  ...
  async onStart(): Promise<void> {
    ...
    const profile: DomainAgentProfile = {
      ...
      capabilities: {
        ...
        rolePrompt: this.config.rolePrompt,
        behaviorRules: this.config.behaviorRules,
        capabilityBoundary: this.config.capabilityBoundary,
...
// 格式示例:
// [
//   {"id": "da_py_01", "name": "PythonDataPro", "skills": ["python", "pandas", "matplotlib"], "tools": ["read_csv", "run_script"]},
//   {"id": "da_web_02", "name": "WebResearcher", "skills": ["browsing", "search"], "tools": ["google_search", "scrape"],}
// ]
...
3. **依赖管理**：明确定义 `dependencies`。
4. **异常上报**：若遇到无法分配的任务（如需要联网但所有 Python Agent 都禁止联网），不要强行分配，填入 `unassignable_tasks`。
...
private validateToolParams(toolName: string, args: any): boolean {
  ...
  switch (toolName) {
    ...
    case 'exec_shell':
      // 仅拒绝含高危 shell 元字符的命令串（与 ExecShellTool 一致）
      if (typeof args.command !== 'string') return false;
      const dangerousChars = ['|', '&', ';', '`', '$', '(', ')', '{', '}', '\n', '\r'];
      return !dangerousChars.some((c) => args.command.includes(c));
...
- 工具调用规则：
  - 工具脚本放置在 `scripts/` 目录下，文件名需与 `scripts` 声明中的 `name` 一致。
  - JavaScript 工具：`node tools/<toolName>.js <argsJson>`
  - Python 工具：`python tools/<toolName>.py <argsJson>`
  - 工具脚本需从命令行参数读取 JSON 参数，执行后输出 JSON 结果到 stdout。
```ts
...
- 目录结构标准：
  ...
  - `skill-name/package.json`：可选，JavaScript 运行时依赖定义。
  - `skill-name/requirements.txt`：可选，Python 运行时依赖定义。
- `SKILL.md` 字段约束：
  ...
  - `allowedtools` 为可选的预批准工具清单，仅用于声明，不绕过系统权限裁决。
- 依赖安装规则：
  - JavaScript：检测到 `package.json` 时执行 `npm install`，清理时删除 `node_modules`。
  - Python：检测到 `requirements.txt` 时执行 `pip install -r requirements.txt`，清理时删除 `venv`。
- 安装拒绝条件：
...
- 目标：保证 Skill 导入、覆盖更新、运行时校验，回滚和卸载路径均可审计、可恢复、可与 ToolHub/SkillRuntimeManager 联动。

- **工具注册机制**：
  - Skill 工具通过继承 `BaseTool` 实现，与系统内置工具（如 `FileReadTool`、`ExecShellTool`）使用相同接口。
  - **逻辑名与执行绑定分离（与 MCP / OpenAPI operationId 同类思路）**：大模型与 API 仅见 `skill:{skillDisplayName}:{logicalName}`；`tool_manifest_json` 中每条工具除 `name`、`description` 外必须带 **`invoke`**（托管运行时专用，不必下发给模型），描述如何启动进程，例如 `{"kind":"script","entry":"scripts/foo.js"}` 或 `{"kind":"cli_dispatch","entry":"scripts/cli.js","subcommand":"search"}`。`SkillRuntimeManager` 只消费 `invoke`，**禁止**再用 `logicalName` 拼接脚本路径。
  - 工具类 `SkillToolImpl` 封装 `SkillRuntimeManager.invokeTool(skillId, invoke, args)`，返回统一的 `ToolResult`。
  - 工具名称格式：`skill:{skillDisplayName}:{logicalName}`（与 DB `skills.name` 一致），确保全局唯一。
  - **作者显式清单（推荐）**：包根目录可选 `skill-tools.json`，`tools[]` 中逐项声明 `name`、`description`、`invoke`；安装时优先采用并做路径存在性校验。无该文件时，由扫描得到的 `scripts/*.js|.py` 列表约束 LLM 生成 `invoke.entry`，或退化为「一脚本一工具」自动发现。
  - 注册时调用 `toolHub.register(skillTool)`，注销时调用 `toolHub.unregisterTool()`；卸载时按 **`skills.name`（展示名）** 注销 `skill:*` 工具，与安装注册键一致。

- **预览流程**（HTTP `POST .../skills/preview`，zip 正文上传）：
  1. 解压 zip 到工作空间下**持久暂存目录** `skills/temp/previews/<previewId>`（`previewId` 为 UUID，响应体带回）。
  2. 递归查找 SKILL.md 所在目录（支持 zip 内含子目录）。
  3. 调用 `SkillRegistry.previewPackage(..., { materializeSkillToolsJson: true })`：执行 `extractToolManifest`，将结果写入该 skill 根目录下的 **`skill-tools.json`**，并在 JSON 响应中返回 `previewId`、元数据与 `tools` 列表。
  4. **不删除**上述暂存目录，供后续安装直接使用同一解压树。
- **安装流程**（HTTP `POST .../skills/import`）：
  - **推荐**：请求体仅带 `previewId`，服务端从 `skills/temp/previews/<previewId>` 定位 skill 根（与预览为同一份解压内容），`install` 将树复制到正式 `skills/<skillId>`；`extractToolManifest` 读取已物化的 `skill-tools.json`，**不再**重复跑预览阶段的 LLM。成功后删除该 `previewId` 暂存目录。
  - **兼容**：无 `previewId` 时仍可上传 zip 正文，服务端临时解压并安装（与旧行为一致），安装完成后删除该次临时解压目录。

- **安装流程** (`install`)：
  1. 执行预览流程获取元数据
  2. 根据 name 生成 skillId（normalizeSkillId）
  3. 查找是否已安装同名 Skill（支持覆盖更新）
  4. 如已安装，创建备份目录
  5. 删除目标目录（如存在）
  6. 复制 Skill 包到目标目录
  7. 如有运行时语言声明或检测到依赖文件，执行依赖安装
  8. 提取工具清单，创建 `SkillToolImpl` 实例并注册到 ToolHub
  9. 更新数据库（upsert）
  10. 清理备份目录

- **卸载流程** (`uninstall`)：
  1. 查询 Skill 信息
  2. 清理运行时产物（node_modules 或 venv）
  3. 从 ToolHub 注销工具
  4. 停止运行时进程（如运行中）
  5. 删除目标目录
  6. 删除数据库记录

- **回滚机制**：安装失败时，如存在备份则恢复原版本
```ts
...
```
- 前端/后端联动约束：
  - 用户选择 `.zip` 后，前端必须先调用预览接口并展示 `name`、`description`、`version`、`author`、`runtimeLanguage`、`tools`，确认后才允许执行正式导入。
  - Skills 列表描述字段只读取 `SKILL.md.description`，不能回退为首行文本或首段截断内容。
  - 卸载失败错误必须绑定到对应 Skill item，不允许只在页面顶部给全局提示。
  - Skills 列表在版本标签后展示"检测到语言"标签（detectedLanguage），仅在检测结果与声明不一致时显示。
...
- `POST /api/v1/agents`
  - Request: `{ name, modelConfig, rolePrompt, behaviorRules, capabilityBoundary, skills: [], knowledgeBaseIds: [] }`
...
- `GET /api/v1/agents/capabilities`
  ...
  - Logic: 查询 `agents` 上的 `capability_status` 等字段；内置工具列表取当前进程 `ToolHub` 注册名；Skill 名称由 `agent_skills`→`skills` 解析；可与内存缓存合并返回实时状态。

### 5.3 模型与 Skill 资源 API
- 模型管理：
  - `GET /api/v1/models`
...
- `POST /api/v1/auth/decision`
  ...
  - Logic:
    - 由前端会话页（`ChatView`）在用户点击「允许 / 拒绝」后调用（当前 P0：内嵌于对话消息流底部的授权提示卡片，**非**全屏模态框；可选「始终允许/始终拒绝」等持久化入口可后续扩展）；
    - 后端 `AuthService` 根据 `authId` 在内存挂起表中完成对应的 `resolve/reject`（与 `ToolHub` → `securitySandbox.intercept` 的 ASK 分支衔接）；
...
- 交互逻辑：
  ...
  - 收到 `AUTH_REQUEST` 事件 -> 在**会话消息流内**渲染授权提示卡片（工具名、参数摘要、允许/拒绝）-> 用户决策 -> 发送 `POST /api/v1/auth/decision`；实现上建议使用独立 WebSocket 订阅（如 `useAuthRequests`），避免仅依赖「最后一条消息」导致授权事件被其它推送覆盖。
  - 历史浏览与逻辑会话切片：
...
- Skill 管理交互细节：
  - `SkillConfig` 中导入 Skill 时，用户先选择 `.zip` 文件，界面调用 `POST /api/v1/skills/preview`：服务端解压至 `skills/temp/previews/<previewId>` 并物化 `skill-tools.json`，响应含 `previewId`、元数据与 `tools`；确认导入时优先 `POST /api/v1/skills/import` 仅传 `previewId`（与 4.6.6 预览/安装链路一致）。
...
| 异常场景       | 触发条件                         | 处理逻辑                                                              | 用户反馈                                       |
|----------------|----------------------------------|-----------------------------------------------------------------------|------------------------------------------------|
| 模型连接失败   | LLM API 超时或 5xx           | 指数退避重试 3 次 -> 标记任务 EXCEPTION -> 释放资源                  | Toast: "模型服务不可用，请检查设置"          |
...
#### 8.2.3 Shell 类工具配置（`exec_shell`）
- 访问控制以工具策略（`DENY` / `ASK` / `ALLOW`）为主。
- 实现侧可对整段 `command` 做危险字符过滤，并在工作目录下执行、限制超时（如 30s）。示例如下：
  ...
  private validateToolParams(toolName: string, args: any, config: ToolConfig): boolean {
    switch (toolName) {
      ...
      case 'exec_shell':
        if (typeof args.command !== 'string') return false;
        const shellDanger = ['|', '&', ';', '`', '$', '(', ')', '{', '}', '\n', '\r'];
        return !shellDanger.some((c) => args.command.includes(c));
...
-- 内置七类基础工具的默认策略由代码维护，不在此表预置行；仅覆盖默认或 Skill 工具策略时才 INSERT/UPDATE。
```
...
### 8.8 前端交互：会话内 ASK 授权卡片（替代独立 AuthModal）
当 `AUTH_REQUEST` 经 WebSocket 送达时，在**对话主区域**展示授权区块（与澄清提示类似，主链路闭环在会话页完成），**不**使用全屏模态层遮挡整个 ChatWindow。
- 展示位置：`MessageList` 底部（或当前任务上下文下方），与消息列表同滚动容器，便于用户对照上下文决策。
- 内容要素：`authId`（仅用于提交决策，可弱化展示）、工具名、`taskId`（若有）、参数 JSON（与 `authService` 推送一致，已脱敏敏感键）、服务端给出的截止时间提示。
- 操作：「拒绝」「允许」-> `POST /api/v1/auth/decision`；多条请求排队时，实现上应逐条展示或队列处理，避免丢失。
- 可选扩展：倒计时 UI、「始终允许/始终拒绝」与 `persistPolicy` / `policyForAgent` 联动（见 5.6）。
- 超时：与后端 `AuthService` 超时一致，超时后任务侧收到拒绝；前端可补充系统消息或 Toast：“授权等待超时，任务已终止”（若需与后端事件对齐可再订阅任务状态变更）。
