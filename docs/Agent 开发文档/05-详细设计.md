...
CREATE TABLE agent_capabilities (
  ...
  role_prompt TEXT,          -- 角色定位 Prompt
  behavior_rules TEXT,       -- 行为规范
  capability_boundary TEXT,  -- 能力边界
...
#### 3.1.5 `agent_tool_permissions`（基础工具权限配置）表
```sql
...
- **工具注册机制**：
  ...
  - 工具类 `SkillToolImpl` 封装 `SkillRuntimeManager.invokeTool()` 调用，返回统一的 `ToolResult`。
  - 工具名称格式：`skill:{skillId}:{toolName}`，确保全局唯一。
  - 注册时调用 `toolHub.register(skillTool)`，注销时调用 `toolHub.unregisterTool()`。

- **预览流程** (`previewPackage`)：
  1. 解压 zip 到临时目录
  2. 递归查找 SKILL.md 所在目录（支持 zip 内含子目录）
  3. 解析 SKILL.md 获取 name、description、version、author、runtime
  4. 提取 tools 声明
  5. 自动检测运行时语言（package.json → javascript，requirements.txt → python）
  6. 返回预览信息（包含 tools 数组）
...
- 组件状态：
 - visible: boolean
 - request: { authId, toolName, args, agentName }
