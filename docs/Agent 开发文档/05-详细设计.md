...
| 项目 | 内容 |
|------|------|
| 职责 | 管理默认会话上下文、消息持久化，任务状态和结果的更新显示， 定时任务状态和结果推送显示 |
...
- 用户发送消息时写入 `Message`，并关联当前会话和可选任务。
- 若消息承载的是权限确认、高风险执行确认或人工介入处置请求，会话层还需关联统一的任务事实摘要引用。
...
- 创建 Agent 时仅允许 Domain 类型；创建后类型不可修改。
- Domain Agent 的创建与编辑流程均包含 Prompt 模板与版本绑定配置，从Prompt 模板里截取Agent描述。
- 管理 Agent 与模型、Skills、权限策略的绑定关系。
- Skill 需要在工具声明中携带权限声明（read/write/execute）；Agent 绑定 Skill 时同步配置该声明权限，默认 ask。
...
- **Domain Agent 管理流程**：
  ...
  2. 创建时固定 `type=domain`；编辑时禁止修改已有 Agent 类型
  3. 校验 Agent 名称、绑定模型、Skills 与 Prompt 模板版本绑定
  4. 若为新建 Agent，保存 Agent 基础配置与 Prompt 绑定后即可按状态启用（模型可用性由设置页模型连通性测试保障）
  5. 若为编辑 Agent，保存后新任务立即生效，运行中任务不受影响
  6. 删除规则：仅 Leader Agent 受保护不可删除；Domain Agent 采用彻底删除（物理删除）
  7. 删除前校验：若存在运行中任务引用当前 Agent（任务分配或运行步骤引用），必须拒绝删除并返回引用任务信息
  8. 停职规则：Domain Agent 支持停职（status=inactive），停职后不可参与新任务执行；运行中任务不受影响
  9. 重新启用规则：停职后的 Domain Agent 支持重新启用（status=active），启用后可再次参与新任务执行
...
- **Agent 与 Skills 绑定**：
  ...
  3. 每个 Skill 的工具声明必须包含权限声明（read/write/execute）并向用户展示所需权限
  4. Agent 绑定 Skill 时，系统按 Skill 工具声明生成 Agent-Skill-工具权限绑定，默认 ask，可逐项调整
  5. 权限检查优先读取 Agent-Skill-工具绑定，再叠加其它策略
  6. 调用记录需按 Agent 独立追溯

边界与约束：

- Leader Agent 始终存在且不可删除
- 创建入口仅允许创建 Domain Agent；Leader Agent 仅可由系统初始化链路创建
- Agent 类型在创建后不可变更
- 除 Leader Agent 外，Domain Agent 允许删除（包括系统预置 Domain Agent），删除为不可恢复的物理删除
- Domain Agent 被运行中任务引用时禁止删除，需先等待相关任务结束
- Domain Agent 支持停职（inactive），停职后不可用于新任务路由和执行
- 停职后的 Domain Agent 支持重新启用（active），启用后恢复可用状态
- Domain Agent 的创建与编辑必须支持 Prompt 模板与版本绑定，模板类型需与 Agent 类型一致，Agent 不再维护角色描述字段
- 系统默认工具权限不再作为独立配置入口；工具权限由 Skill 声明与 Agent-Skill 绑定配置驱动
...
| ExecutionAttachmentContext | fileName, parseSummary | 附件解析上下文（包含文件名和内容/路径） |
...
- **意图解析流程**：
  1. 调用 LLM 进行意图分析，输入包含：
     - 用户原始输入
     - 附件列表（文件名）
     - 系统中可用的 Leader Agent 和 Domain Agent 列表（包含 ID 和名称）
  2. LLM 返回 JSON 格式的决策结果：
     - `complexity`: "simple" 或 "complex"
     - `summary`: 任务分析摘要
     - `steps`: 步骤列表，包含 `name`、`agentId`、`isBlocking`
  3. 若 LLM 调用失败，回退到简单分析（使用 Leader Agent 单步执行）
  4. 生成 `TaskRouteDecision`，包含 complexityDecisionSummary

- **附件解析流程**：
  1. 调用 `resolveTaskAttachments(conversationId)` 获取会话中的附件
  2. 对每个附件：
     - 文本文件：解码 base64 内容
     - 二进制文件：保存到 workspace 目录，调用 `attachmentService.parseAttachment()` 解析
  3. 返回 `ExecutionAttachmentContext` 列表，包含文件名和内容/路径
  4. 附件信息通过 `PromptContext.attachments` 传递给 Agent

- **Agent 选择**：
  1. 从 `agentService.getLeaderAgents()` 获取 Leader Agent
  2. 从 `agentService.getDomainAgents()` 获取 Domain Agent 列表
  3. LLM 返回的 `agentId` 直接使用数据库中的真实 UUID
  4. 若 LLM 返回的 agentId 无法匹配，使用默认 Leader Agent

- **步骤生成流程**：
  1. 根据 `complexity` 决定步骤数量：
     - simple：使用 LLM 返回的步骤或单步 Leader 执行
     - complex：使用 LLM 返回的步骤或基于 skill 生成多步计划
  2. 每个步骤包含：
     - `agentId`: 真实的 Agent UUID
     - `name`: 步骤名称
     - `order`: 执行顺序
     - `isBlocking`: 是否阻塞后续步骤
...
- 意图解析使用 LLM 时，必须先获取系统中实际配置的 Agent 列表，不使用硬编码的假 Agent ID。
- 附件内容不嵌入 Prompt，而是通过文件路径由 Agent 主动读取。
...
| 入口依赖 | 用户上传的附件 |
| 出口目标 | 任务模块（InputSlice）、编排模块（附件路径解析） |

职责：

- 负责上传、存储、解析、重解析和附件状态管理。
- 为任务输入标准化提供文本切片、OCR 结果和解析摘要。
- 为 Agent 执行提供附件路径解析服务。
...
- **上传流程**：
  1. 用户上传附件，文件以 base64 data URL 形式存储在消息中
  2. 同时写入 `attachments` 表记录元数据

- **解析流程**：
  1. 根据文件类型选择解析器：
     - 文本文件 (txt, md, json, xml) → 直接解码
     - PDF → pdf-parse 库
     - Word (.doc/.docx) → mammoth 库
     - 图片 → OCR 识别
  2. 解析成功后提取文本内容，存入 `parseSummary` 字段

- **编排模块附件解析**：
  1. 编排模块通过 `resolveTaskAttachments(conversationId)` 方法获取会话中的附件
  2. 解析附件 data URL，根据 MIME 类型决定处理方式：
     - 文本文件：直接解码 base64 内容
     - 二进制文件：保存到 workspace 目录，调用 `attachmentService.parseAttachment()` 解析
  3. 返回 `ExecutionAttachmentContext` 列表，包含文件名和内容/路径

- **ExecutionAttachmentContext 结构**：
  ```
  ExecutionAttachmentContext {
    fileName: string        // 文件名
    parseSummary: string   // 解析后的文本内容或文件路径
  }
  ```

- **附件到工作目录的映射**：
  1. 文本文件：解码后以原文件名保存到 workspace（如 `01-需求分析.md`）
  2. 二进制文件：解析后的文本内容以原文件名保存
  3. Agent 通过 `read_file` 工具读取工作目录中的文件

- **附件路径传递到 Agent**：
  1. 在 `PromptContext.attachments` 中填充附件信息
  2. Prompt 模板中使用 `{{attachments}}` 循环输出文件名和路径
  3. Agent 根据文件名使用 `read_file` 工具读取内容

边界与约束：

- 附件模块只负责输入处理，不负责知识沉淀决策。
- 附件入知识库必须由显式用户动作或明确的系统流程触发。
- 附件内容不直接嵌入 Prompt，而是通过文件路径由 Agent 主动读取。
- workspace 目录路径由 `config.workspace.path` 配置，默认 `./workspace`。
...
**Skill 标准**
基于 https://agentskills.io/specification 的扩展
- 目录结构标准：
  skill-name/
  ...
  ├── permissions.json                             # Optional: permission policies
  ├── install.xx                                   # Optional: installation script
  ├── uninstall.xx                                 # Optional: uninstallation script
  └── package.json | requirements.txt              # Optional: dependencies
- SKILL.md 字段说明：
  ...
  metadata	                                       # Optional	Arbitrary key-value mapping for additional metadata.
    - author: example-org                          # Optional	Author or owning organization of the skill.
    - version: "1.0"                               # Optional	Version number following semantic versioning.
    - language: JavaScript                         # Optional	Declared runtime language for lifecycle scripts and execution consistency.
...
- Skill 目录结构：
  ...
  ```
- **安装流程**：
  1. 通过管理页从文件系统选择 Skill `.zip` 安装包；选包后先调用 `POST /api/skills/preview` 预读取并展示安装包元数据
  2. 预读取元数据字段包含：`name`、`description`、`version`、`author`（`license` 仅作为 SKILL.md 引用，不参与预读取与展示）
  3. 用户确认后上传到 `POST /api/skills/import`
  4. 服务端解压安装包，定位包含 `SKILL.md` 的 Skill 根目录；若缺失则导入失败
  5. 将安装包内容落盘到 `skills/<skill-id>/`（`skill-id` 由包名或目录名规范化生成）
  6. 读取 `SKILL.md` 解析 Skill 元数据
  7. 当声明了 `runtimeLanguage` 时，校验单包内脚本、依赖文件、执行器与该语言一致；若检测到多语言混装或声明与内容不一致则拒绝安装
  8. 当未声明 `runtimeLanguage` 时，进入“仅目录拷贝模式”，跳过依赖安装与 install 脚本执行
  9. 根据 `runtimeLanguage`（若已声明）安装对应依赖：
    - JavaScript → `npm install`
    - Python → `pip install -r requirements.txt`
    - Ruby → `bundle install`
  10. 执行安装脚本（如存在且已声明运行时），传入参数：通过文件传递 `{ skillId, skillPath, config }`
  11. 将 Skill 注册到系统数据库并返回导入结果
  12. Skills 管理页不提供独立“更新”按钮；重复导入同一 `skill-id` 的 zip 安装包即视为版本更新
  13. 安装过程中的运行时校验失败时，系统必须回滚本次导入创建的目录与记录（若为覆盖更新则恢复旧目录），不保留“校验失败”的残留 Skill 条目
- **卸载流程**：
  1. 从系统数据库注销 Skill
  2. 当声明了 `runtimeLanguage` 时：执行卸载脚本（如存在）并清理对应依赖目录（`node_modules`/`venv`/`.bundle`）
  3. 当未声明 `runtimeLanguage` 时：仅执行 Skill 目录删除，不查找卸载脚本
- **前端交互约束**：
  - 用户选择 `.zip` 安装包后，导入区域需先展示从 `SKILL.md` 预读取的 `name`、`description`、`version`、`author`，再由用户确认执行导入。
  - Skills 列表中 Skill item 的描述字段必须读取 `SKILL.md` 的 `description` 字段，不得回退为文件首行（如 `---`）或首段截断文本。
  - `license` 在 `SKILL.md` 中仅作引用信息，导入预览与 Skills 列表均不读取、不显示该字段。
  - Skills 管理页触发“卸载”失败时，错误提示必须显示在对应 Skill item 内（item-level error），不得仅显示页面级全局错误。
  - 同一 Skill 再次触发“卸载”前，前一次该 Skill 的 item 级错误提示应先清空，避免陈旧错误误导。
  - Skills 列表不展示运行时校验明细块（错误码/详情/维度/检测语言明细）；仅在版本标签后展示“检测到语言”标签，且仅当检测结果非空时显示。
  - Skills 的运行时/版本/状态信息仅通过标题后的标签展示，不再在 item 内容区单独占一行。
  - Skills 管理页不提供“校验失败”筛选项；校验失败由导入流程即时失败并回滚处理，不在列表中形成可筛选残留项。
- **支持的语言**：
...
| 项目 | 内容 |
|------|------|
| 职责 | 提供内置读/写/编辑/执行命令能力，统一参数校验、权限拦截、沙箱执行与审计落库 |
| 核心对象 | ToolCallRequest, ToolValidationResult, SandboxExecutionContext, ToolExecutionResult |
| 入口依赖 | 编排模块的工具调用决策、Permission Policy Service 的权限决策 |
| 出口目标 | 任务模块（步骤推进）、审计模块（工具调用日志） |

职责：

- 为 Agent 提供统一的四类内置工具：`read_file`、`write_file`、`edit_file`、`execute_command`。
- 对工具调用执行统一处理链：参数校验 -> 权限判定 -> 路径与命令安全校验 -> 沙箱执行 -> 结果归一化。
- 将工具输入参数摘要、执行耗时、执行结果与错误码写入 `ToolInvocationLog`。
...
ToolCallRequest {
  ...
  stepId: string
  agentId: string
  toolName: 'read_file' | 'write_file' | 'edit_file' | 'execute_command'
...
1. 编排模块发起 `ToolCallRequest`。
2. 工具模块按 `toolName` 执行参数 schema 校验与默认值补齐。
3. 根据动作类型执行权限判定：
   - `read_file`：默认按 `read` 动作判定
   - `write_file`、`edit_file`：按 `write` 动作判定
   - `execute_command`：按 `execute` 动作判定
4. 若权限结果为 `allow`，进入沙箱执行；`ask` 返回权限请求；`deny` 返回拒绝结果并终止。
5. 沙箱执行后统一归一化返回，写入 `ToolInvocationLog` 并回传编排模块用于步骤推进。

四类内置工具详细设计：

- `read_file`
  - 入参：
    - `path`（必填，工作区相对路径或受控绝对路径）
    - `startLine`（可选，默认 `1`）
    - `lineCount`（可选，默认 `200`，最大 `2000`）
  - 行为：按窗口返回文本片段，支持从指定位置滑动读取；目录或不存在路径返回错误
  - 返回：`content`、`hasMore`
  - 约束：
    - 文件大小不得超过 `maxOutputSize`，超限返回 `TOOL_FILE_TOO_LARGE`
    - `startLine/lineCount` 非法返回 `TOOL_RANGE_INVALID`

- `write_file`
  - 入参：`path`（必填）、`content`（必填）
  - 行为：创建父目录（若不存在）并覆盖写入目标文件
  - 约束：禁止写入可执行后缀文件（如 `.exe/.sh/.bat/.ps1/.py/.js`），命中返回 `TOOL_WRITE_BLOCKED`

- `edit_file`
  - 入参：
    - `path`（必填）
    - 文本替换模式：`oldText`（必填）、`newText`（必填）、`replaceAll`（可选，默认 `false`）
    - 位置编辑模式：`startLine`（必填）、`startColumn`（必填）、`endLine`（可选）、`endColumn`（可选）、`newText`（必填）
  - 行为：
    - 文本替换模式：先读取目标文件，再执行精确文本替换，最后回写文件
    - 位置编辑模式：对指定区间进行替换或插入，支持从某个位置开始连续编辑
  - 约束：
    - 当 `replaceAll=false` 时必须且仅匹配一次，未命中或命中多次返回 `TOOL_EDIT_NOT_UNIQUE`
    - 行列范围非法或分片乱序返回 `TOOL_RANGE_INVALID`
    - 分片写入必须满足幂等与顺序一致性，冲突返回 `TOOL_EDIT_CONFLICT`
    - 与 `write_file` 共享路径与可执行文件写入限制
    - 编辑失败不得产生部分写入，必须保持原文件不变

- `execute_command`
  - 入参：`command`（必填）、`cwd`（可选）
  - 行为：在受控工作目录执行单条命令并返回 stdout/stderr
  - 约束：
    - 禁止高风险命令模式（如命令拼接、重定向、下载器等）
    - 执行超时受 `maxExecutionTimeMs` 控制，超时返回 `TOOL_COMMAND_TIMEOUT`
    - 输出超过 `maxOutputSize` 时截断并标记 `metadata.truncated=true`

错误码与回退策略：

- 参数错误：`TOOL_INVALID_ARGUMENT`
- 路径越界：`TOOL_PATH_DENIED`
- 范围非法：`TOOL_RANGE_INVALID`
- 权限拒绝：`TOOL_PERMISSION_DENIED`
- 权限待确认：`TOOL_PERMISSION_REQUIRED`
- 编辑冲突：`TOOL_EDIT_CONFLICT`
- 命令执行失败：`TOOL_COMMAND_FAILED`
- 内部异常：`TOOL_INTERNAL_ERROR`

当工具执行失败时，任务步骤不直接崩溃：

- 可恢复错误（如临时 IO 失败）进入有限重试
- 不可恢复错误（如权限拒绝、路径越界）按拒绝分支推进并写入可读摘要
- 每次失败都必须产出审计日志和任务事件，保证可追溯
...
| 核心对象 | PromptTemplate, PromptVersion, AgentPromptProfile, PromptMigrationRecord, PromptContext |
...
- **Prompt 模板结构**：
  ...
  PromptVersion {
    version: number
    system: string      // 系统角色内容
    user: string        // 用户层内容
    context: string     // 上下文内容
    toolResult: string  // 工具结果内容
  ...
  AgentPromptProfile {
    ...
    roleDefinition: string    // 角色定位定义
    behaviorNorm: string      // 行为规范
    capabilityBoundary: string // 能力边界
    updatedAt: datetime
  }
  ...
  PromptMigrationRecord {
    ...
    compatibilityResult: enum  // compatible/incompatible/warning
    migratedAt: datetime
  }
  ```

- **上下文装配流程**：
  1. 接收任务上下文请求 (`PromptContext`)
  2. 读取当前 Agent 的 `AgentPromptProfile` 与绑定模板版本
  3. 若 Agent 无 Profile，使用默认模板：
     - Leader Agent → `leader-v1` 模板
     - Domain Agent → `domain-v1` 模板
  4. 按以下顺序装配上下文：
     - 系统角色（system）：填入 roleDefinition、behaviorNorm、capabilityBoundary
     - 任务描述（user）：
       - 填入 input（用户原始输入）
       - 附件信息（attachments）：输出文件名列表，Agent 通过 read_file 工具读取
       - 已完成步骤（previousSteps）
     - 上下文（context）：
       - 任务名称（taskName）
       ...
       - 短期记忆（memory）
       - 知识库命中（knowledge）
  5. 使用 Handlebars 语法处理条件渲染：
     - `{{#if attachments}}...{{/if}}`：条件渲染
     - `{{#each attachments}}...{{/each}}`：循环渲染
     - `{{this.fileName}}`：当前元素属性
...
- **版本控制流程**：
  1. Prompt 模板管理页支持模板创建、修改、删除。
  2. 创建模板与编辑模板只提供一个 Prompt 内容文本框（单一文本内容）。
  3. 模板列表每个 item 提供"修改模板/删除模板"操作入口；删除需二次确认并提示不可恢复。
  4. 编辑模板保存时，如果 Prompt 内容发生变化，系统自动创建新版本并更新当前版本号。
  5. 版本历史需可审计（版本号、自动或人工变更原因、创建时间）。
  6. 支持从历史版本恢复；恢复动作会以"恢复到 vX"的说明写入新的版本记录。
  7. Agent 侧仅选择模板名称与版本进行绑定，不再编辑 Prompt 内容。
  8. 任务执行时记录实际使用的模板版本 + Agent Prompt 绑定版本。
...
- **默认模板结构**：
  ...
  **leader-v1 模板**：
  ...
  user: 用户输入：{{input}}
        ...
        {{#if attachments}}附件文件（请使用 read_file 工具读取内容）：
  ...
  **domain-v1 模板**：
  ...
  user: 任务输入：{{input}}
        ...
        请使用适当的工具执行任务。如果任务完成，请调用finish工具。
  ```

边界与约束：

- Prompt 模板修改需保留变更记录
- Agent Prompt 绑定需与模板版本分离追踪
- 模板删除采用受控删除路径，删除后不得继续被 Agent 选择
- 裁剪决策必须可解释、可记录
- 不得把完整原始思维链作为 Prompt 输出
- 任务详情可追溯实际使用的模板内容
- 附件内容不直接嵌入 Prompt，通过文件路径由 Agent 主动读取
...
1. 前端提交 multipart 文件到 `/api/knowledge/upload`。
2. 后端按扩展名执行解析（pdf/doc/docx/text）。
3. 调用 LLM 生成 `title` 与 `overview`。
4. 文档正文写入 `knowledge_documents` 主索引。
5. 文档正文切片并写入 LanceDB，回填分片元数据到 `vector_ids`。

### 8.2 标题编辑时序

1. 前端调用 `PATCH /api/knowledge/:id/title`。
2. 后端校验 `title` 非空并更新 `knowledge_documents.title`。
3. 返回 `success=true`，前端刷新列表。

### 8.3 失败回滚规则

- 若 LanceDB 写入失败，回滚已创建的 SQLite 文档记录。
- 若 LLM 摘要失败，使用文件名与正文前缀执行回退，流程不中断。
