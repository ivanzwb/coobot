...
- 管理 Agent 级 Prompt 模板与版本绑定
- 支持 Prompt 模板版本化
- 支持模板版本恢复（恢复操作写入新的版本记录）
- 处理 Prompt 超长时的裁剪逻辑
- 负责上下文（记忆、知识、工具）的装配
...
- **版本控制流程**：
  ...
  3.1 删除模板前需要校验占用关系：若有 Agent 正在绑定该模板，删除必须被拒绝，并返回占用 Agent 列表。
  4. 编辑模板保存时，如果 Prompt 内容发生变化，系统自动创建新版本并更新当前版本号。
  5. 版本历史需可审计（版本号、自动或人工变更原因、创建时间）。
  5.1 模板列表 item 仅展示模板名、类型和当前版本，不展示状态字段。
  5.2 版本历史点击“查看”后需切换内容预览；首次进入默认预览最新版本内容。
  6. 支持从历史版本恢复；恢复动作会以“恢复到 vX”的说明写入新的版本记录，而不是直接覆盖旧版本。
...
- 若模板仍被 Agent 绑定，必须阻止删除并返回占用 Agent 名单供前端提示。
- 裁剪决策必须可解释、可记录
