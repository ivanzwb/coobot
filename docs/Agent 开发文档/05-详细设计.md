...
- 主要方法：
  ...
  - getAllowedDomains(): string[]：获取 HTTP 白名单
  - getAllowedCommands(): string[]：获取 Shell 白名单
- 内置基础工具集（F03.3 实现）
  | 工具名 | 功能描述 | 权限粒度 | 默认策略 |
  |--------|----------|----------|----------|
  | `read_file` | 读取本地文件 | 路径白名单/文件类型 | ASK |
  | `edit_file` | 编辑本地文件 | 路径白名单/文件类型 | ASK |
  | `write_file` | 写入本地文件 | 路径白名单/文件类型/大小限制 | ASK |
  ...
  | `system_info` | 获取系统信息 | 信息级别过滤 | ALLOW |
...
async intercept(agentId: string, toolName: string, args: any): Promise<void> {
  ...
  // 3. 细粒度参数验证 (F03.3 核心)
  if (!this.validateToolParams(toolName, args, policy)) {
...
// F03.3 细粒度参数验证
private validateToolParams(toolName: string, args: any, policy: string): boolean {
...
class FileReadTool extends BaseTool {
  ...
  async execute(args: { path: string; max_lines?: number }): Promise<string> {
    // 1. 路径安全校验 (双重保险)
    ...
    if (stats.size > 10 * 1024 * 1024) { // 10MB
      throw new Error(`File too large: ${safePath} (${stats.size} bytes)`);
    }
    ...
    return lines.slice(0, args.max_lines || 100).join('\n');
  }
}
```
