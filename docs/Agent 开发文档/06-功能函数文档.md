...
- 单个 Skill 包必须显式声明唯一的 `runtimeLanguage`，同一 Skill 包内不允许混用多种运行时。
...
- `GET /api/prompts`（来源：N-12，获取 Prompt 模板列表）
- `POST /api/prompts`（来源：N-12，创建 Prompt 模板）
- `GET /api/prompts/:id`（来源：N-12，获取 Prompt 模板详情）
- `PATCH /api/prompts/:id`（来源：N-12，更新 Prompt 模板）
- `DELETE /api/prompts/:id`（来源：N-12，删除 Prompt 模板）
- `GET /api/prompts/:id/versions`（来源：N-12，获取 Prompt 版本历史）
- `POST /api/prompts/:id/rollback`（来源：N-12，回滚到指定版本）
- `GET /api/agents/:agentId/prompt-profile`（来源：N-12，获取 Agent 独立 Prompt 配置）
- `PATCH /api/agents/:agentId/prompt-profile`（来源：N-12，更新 Agent 独立 Prompt 配置）
- `POST /api/agents/:agentId/prompt-template-switch`（来源：N-12，切换模板版本并自动迁移）
- `GET /api/agents/:agentId/prompt-migrations`（来源：N-12，获取迁移记录与失败原因）
...
- `CreateSkillRequestDto`：包含 `name`、`description`、`instructions`、`version`、`runtimeLanguage`、`permissions?`、`tools?`、`metadata?`；其中 `runtimeLanguage` 为必填字段，用于显式声明该 Skill 包唯一运行时语言。若请求体缺失该字段，或上传内容表现为多语言混装，则返回 Skill 安装校验错误。（来源：N-11）
- `UpdateSkillRequestDto`：包含 `name?`、`description?`、`instructions?`、`version?`、`runtimeLanguage?`、`permissions?`、`tools?`、`metadata?`；若更新 `runtimeLanguage`，系统必须校验当前 Skill 包脚本、依赖文件和执行器目录与目标语言一致，且不得在执行链路引用期间切换语言。（来源：N-11）
- `ActivateSkillRequestDto`：包含 `agentId`、`taskId?`、`expectedSkillVersion?`；Skill 激活前需确保目标 Skill 已安装、已启用且 `runtimeLanguage` 校验通过。（来源：N-11）
- `CreatePromptTemplateRequestDto`：包含 `name`、`type`（leader/domain/output）、`description?`、`schema`（包含 system、developer、user、context、toolResult、slotSpec）；`type` 为必填字段，用于区分 Prompt 模板类型。模板只定义结构与插槽，不承载具体 Agent 角色内容。（来源：N-12）
...
- `PromptTemplateListResponseDto` / `PromptTemplateDetailResponseDto`：除基础 Prompt 信息外，还需返回 `type`（leader/domain/output）、`currentVersion`、`versions[]`、`schema`、`createdAt`、`updatedAt`；其中 `versions[]` 需包含版本号、变更原因和创建时间。（来源：N-12）
