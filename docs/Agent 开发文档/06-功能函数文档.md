...
- 多端接口语义需区分“消息同步”和“任务事实一致性”：是否跨端同步消息由 `syncPolicy` 等字段表达，但任务状态、事件流、结果终态、权限裁决结果等任务事实必须按统一事实模型返回，不得因消息未同步而在不同接入端返回相互冲突的事实字段。
...
- `POST /api/skills/preview`（来源：N-11，预读取 zip 安装包中的 SKILL.md 元数据）
- `POST /api/skills/import`（来源：N-11，从文件系统导入 zip 安装包）
...
- `POST /api/skills/:id/install`（来源：N-11，安装Skill依赖）
- `POST /api/skills/:id/uninstall`（来源：N-11，卸载Skill依赖）
- `GET /api/skills/:id/status`（来源：N-11，获取安装状态和运行时语言信息）
- Skills 管理页不提供独立“更新”按钮；重复调用 `POST /api/skills/import` 导入同一 `skill-id` 的安装包时，语义即为 Skill 更新。
- Skills 管理页在触发 `POST /api/skills/:id/uninstall` 失败时，必须把错误提示渲染在对应 Skill item 内，不得只在页面顶部显示全局错误，避免列表较长时失败信息不可见。
- 当导入同一 `skill-id` 的安装包且本地 Skill 目录不存在时，后端应按“全新安装”处理，不返回 `Skill directory not found` 错误。
- 安装流程中的运行时校验若失败，后端必须回滚本次导入创建内容（新目录与新增记录）；覆盖更新场景需恢复旧目录，避免产生可见的“校验失败”残留 Skill。

**Skill 多语言支持：**
- 系统整体支持多种编程语言实现的 Skill。
- `runtimeLanguage` 为可选字段；未声明时 Skill 按“仅目录拷贝/删除模式”运行，不执行 install/uninstall 脚本与依赖安装。
- 当声明了 `runtimeLanguage` 时，同一 Skill 包内不允许混用多种运行时。
- 安装脚本支持：`.js`、`.mjs`、`.py`、`.rb`、`.sh`、`.ps1`
- 卸载脚本支持：`.js`、`.mjs`、`.py`、`.rb`、`.sh`、`.ps1`
- 自动依赖安装：支持 `package.json`（npm）、`requirements.txt`（pip）、`Gemfile`（bundle）
- `GET /api/skills`、`GET /api/skills/:id`、`GET /api/skills/:id/status` 的返回体都应包含 `runtimeLanguage`，用于向管理界面、安装状态页和审计链路稳定暴露 Skill 当前声明的运行时语言。
- Skills 管理页对 `GET /api/skills/:id/status` 返回中的运行时校验结果采用精简展示：不显示错误码/详情/维度/检测语言明细块，只在版本标签后显示“检测到语言”标签（`detectedLanguages` 非空时显示）。
- Skills 管理页中的运行时/版本/状态采用标题后标签展示，不在 Skill item 正文区单独渲染一行文本。
- Skills 管理页不提供“校验失败”筛选；校验失败在导入/安装流程中即时返回并回滚，不在列表中保留失败项。
- `POST /api/skills/import` 请求体采用 `multipart/form-data`，字段名支持 `file` 或 `files`；后端从 zip 中识别 `SKILL.md` 并完成导入。
- `POST /api/skills/preview` 请求体采用 `multipart/form-data`，字段名支持 `file` 或 `files`；后端从 zip 中识别 `SKILL.md` 并返回预览字段：`name`、`description`、`version`、`author`。
- Skills 管理页 item 描述字段必须来源于已安装 Skill 的 `SKILL.md` 中 `description` 字段，不得使用文件首行或说明段首行作为回退展示值。
- Skills 预览与列表展示均不读取 `license` 字段；`license` 在 `SKILL.md` 中仅作为引用信息。
...
- `PATCH /api/memories/:id`（来源：N-09，更新记忆；支持编辑 `content`、`summary`、`importance`）

`PATCH /api/memories/:id` 约束：

- 请求体至少包含一个可更新字段：`content`、`summary`、`importance`。
- `content` 与 `summary` 若出现，去除首尾空白后不能为空字符串。
- `importance` 若出现，取值必须为 `low | medium | high`。
- 目标记忆不存在时返回 `404 MEMORY_NOT_FOUND`。

### 2.10 配置接口

- `GET /api/config`（来源：N-14，获取系统配置）
- `PATCH /api/config`（来源：N-14，更新系统配置）
- `GET /api/config/models`（来源：N-06，获取模型列表）
- `POST /api/config/models`（来源：N-06，创建模型配置）
- `PATCH /api/config/models/:id`（来源：N-06，更新模型配置）
- `DELETE /api/config/models/:id`（来源：N-06，删除模型配置）
- `POST /api/config/models/test`（来源：N-06，测试模型连通性）
- `POST /api/config/models/:id/set-default`（来源：N-06，设置默认模型）

**多模型配置说明：**
- 每个模型配置包含以下字段：
  - `id`: 模型唯一标识（如 `gpt-4`、`claude-3-opus`）
  - `name`: 显示名称（如 `GPT-4`）
  - `provider`: 供应商类型（`openai`/`local`/`custom`）
  - `baseUrl`: API 基础地址（可选，用于自定义 API 或代理）
  - `apiKey`: API 密钥（可选，留空则使用全局配置）
  - `defaultTemperature`: 默认温度参数（默认 0.7）
  - `defaultMaxTokens`: 默认最大 token 数（默认 4096）
  - `timeout`: 超时时间（毫秒，默认 120000）
  - `enabled`: 是否启用（默认 true）
- Agent 创建/更新时可指定使用的模型（从已配置的模型中选择）
- 不同 Agent 可以使用不同的模型，实现差异化配置
- 支持设置默认模型，默认模型用于系统级调用
...
- `GET /api/prompts/templates`（来源：N-12，获取 Prompt 模板列表）
- `POST /api/prompts/templates`（来源：N-12，创建 Prompt 模板）
- `GET /api/prompts/templates/:templateId`（来源：N-12，获取 Prompt 模板详情）
- `PATCH /api/prompts/templates/:templateId`（来源：N-12，更新 Prompt 模板元信息）
- `DELETE /api/prompts/templates/:templateId`（来源：N-12，删除 Prompt 模板）
- `GET /api/prompts/templates/:templateId/versions`（来源：N-12，获取 Prompt 版本历史）
- `GET /api/prompts/templates/:templateId/versions/:version`（来源：N-12，获取指定模板版本内容）
- `POST /api/prompts/templates/:templateId/versions`（来源：N-12，创建模板新版本，需包含变更原因）
- `POST /api/prompts/templates/:templateId/rollback`（来源：N-12，恢复到指定版本并生成新版本记录）
- `GET /api/prompts/profiles/:agentId`（来源：N-12，获取 Agent 模板绑定）
- `POST /api/prompts/profiles/:agentId`（来源：N-12，创建 Agent 模板绑定）
- `PATCH /api/prompts/profiles/:agentId`（来源：N-12，更新 Agent 模板绑定）

**Prompt 模板版本约束：**
- 创建模板与编辑模板在页面上仅提供一个 Prompt 内容输入框。
- 模板列表每个 item 提供“修改模板/删除模板”动作；删除前需确认并提示删除后不可恢复。
- Prompt 模板管理页搜索范围仅包含 `name`、`type`，不包含描述与正文内容。
- 搜索结果命中关键字时，列表中的命中片段必须高亮显示。
- 删除模板前必须校验占用关系；若模板被一个或多个 Agent 使用，接口返回冲突并附占用 Agent 列表。
- 编辑模板时若 Prompt 内容有变化，系统自动创建新版本并更新 `currentVersion`。
- 模板列表 item 仅展示 `name`、`type`、`currentVersion`，前端不展示模板状态字段。
- 版本历史点击“查看”需更新内容预览；默认预览最新版本内容。
- 执行恢复时必须指定历史 `version`，系统会复制该版本内容并创建新的版本记录。
- Agent 侧仅进行模板名称与版本绑定，不再编辑 Prompt 文本。
- Agent Prompt 绑定时必须满足类型约束：Leader Agent 仅可绑定 leader 模板，Domain Agent 仅可绑定 domain 模板；
- Agent 管理页中，Prompt 绑定配置并入 Agent 编辑弹窗：点击保存时仅调用一次 `PATCH /api/agents/:id`，并在请求体中携带 `promptProfile`（`templateId`、`templateVersion`）；服务端在同一请求内完成 Agent 基础字段更新与 Prompt 绑定更新/创建。

**Usability Common（通用可用性）要求：**
- 所有管理类页面在提供关键词检索时，命中结果应在列表中进行关键字高亮，便于用户快速定位。
- Prompt 模板管理页作为首个落地点，要求“仅按模板名与类型搜索 + 命中高亮”作为基线实现。

### 2.14 检查点恢复接口

- `GET /api/tasks/:id/checkpoints`（来源：N-14.2，获取任务检查点列表）
- `GET /api/tasks/:id/checkpoints/:checkpointId`（来源：N-14.2，获取检查点详情）
- `POST /api/tasks/:id/checkpoints/:checkpointId/recover`（来源：N-14.2，从检查点恢复任务）
- `GET /api/tasks/:id/checkpoints/:checkpointId/validate`（来源：N-14.2，验证检查点有效性）

### 2.15 触发评估接口

- `GET /api/tasks/:id/trigger-evaluation`（来源：N-01.2，N-14，获取触发评估状态）
- `POST /api/tasks/:id/trigger-evaluation/re-evaluate`（来源：N-14，手动触发重评估）
...
- `RollbackPromptVersionRequestDto`
- `UpdateAgentPromptProfileRequestDto`
- `SwitchAgentPromptTemplateVersionRequestDto`
- `RecoverFromCheckpointRequestDto`
- `ValidateCheckpointRequestDto`
- `ReevaluateTriggerRequestDto`
- `CreateAgentRequestDto`
- `UpdateAgentRequestDto`
...
- `CreateAgentRequestDto`：包含 `name`、`type`（leader/domain）、`role?`、`model`、`temperature?`、`skills?`、`knowledgeBases?`；其中 `model` 为必填字段，且只能从 `GET /api/config/models` 返回且 `enabled=true` 的模型中选择。（来源：N-06，N-07）
- `UpdateAgentRequestDto`：包含 `name?`、`role?`、`model?`、`temperature?`、`skills?`、`knowledgeBases?`、`status?`；若更新 `model`，必须校验目标模型可用且启用，校验失败返回 `400 INVALID_MODEL_SELECTION`。（来源：N-06，N-07）
- `CreateSkillRequestDto`：包含 `name`、`description`、`instructions`、`version`、`runtimeLanguage?`、`permissions?`、`tools?`、`metadata?`；其中 `runtimeLanguage` 为可选字段。未声明时系统仅执行目录拷贝/删除，不执行脚本安装链路；若声明后出现多语言混装或声明与包内容不一致，则返回 Skill 安装校验错误。（来源：N-11）
- `ImportSkillPackageRequestDto`：`multipart/form-data` 请求，至少包含 `file`（或兼容字段 `files`）且文件必须为 `.zip`；安装包内必须包含 `SKILL.md`。导入成功返回 `skillId` 与成功消息，失败返回导入错误码与原因。（来源：N-11）
- `UpdateSkillRequestDto`：包含 `name?`、`description?`、`instructions?`、`version?`、`runtimeLanguage?`、`permissions?`、`tools?`、`metadata?`；`runtimeLanguage` 可设置为具体语言或清空。若设置为具体语言，系统必须校验当前 Skill 包脚本、依赖文件和执行器目录与目标语言一致。（来源：N-11）
- `ActivateSkillRequestDto`：包含 `agentId`、`taskId?`、`expectedSkillVersion?`；Skill 激活前需确保目标 Skill 已安装且已启用；若声明了 `runtimeLanguage`，还需通过对应运行时一致性校验。（来源：N-11）
- `CreatePromptTemplateRequestDto`：包含 `name`、`type`（leader/domain）、`description?`、`schema`（包含 system、developer、user、context、toolResult、slotSpec）；`type` 为必填字段，用于区分 Prompt 模板类型。模板只定义结构与插槽，不承载具体 Agent 角色内容。（来源：N-12）
- `UpdatePromptTemplateRequestDto`：包含 `name?`、`description?`、`schema?`、`version?`、`changeReason?`；每次更新自动创建新版本，记录变更原因。（来源：N-12）
- `RollbackPromptVersionRequestDto`：包含 `targetVersion`、`reason?`；只能回滚到已存在的版本，回滚后新任务使用新版本，运行中任务不受影响。（来源：N-12）
- `UpdateAgentPromptProfileRequestDto`：包含 `templateId`、`templateVersion`、`roleDefinition`、`behaviorRules`、`capabilityBoundaries`、`customSections?`、`changeReason?`；用于维护 Agent 独立 Prompt 内容与模板绑定关系。（来源：N-12）
- `SwitchAgentPromptTemplateVersionRequestDto`：包含 `targetTemplateVersion`、`migrationMode?`、`changeReason?`；调用后系统自动执行兼容校验、插槽映射补全与冲突标记，失败时返回迁移错误并保持原版本生效。（来源：N-12，N-14）
- `RecoverFromCheckpointRequestDto`：包含 `checkpointId`、`recoveryType`（continue_from_checkpoint/restart_from_beginning）、`expectedAggregateVersion?`；用于从检查点恢复任务执行。（来源：N-14.2）
- `ValidateCheckpointRequestDto`：包含 `checkpointId`；用于验证检查点的完整性、幂等性和时效性。（来源：N-14.2）
- `ReevaluateTriggerRequestDto`：包含 `taskId`、`force?`；用于手动触发等待条件的重新评估。（来源：N-14）

`CreateAgentRequestDto` / `UpdateAgentRequestDto` 模型选择约束：

- 前端创建/编辑 Agent 时必须展示模型下拉框，数据来源统一使用 `GET /api/config/models`。
- 下拉框仅可选择 `enabled=true` 的模型；禁用模型不得作为可选项。
- 当请求体中的 `model` 不存在或已禁用时，服务端返回 `400 INVALID_MODEL_SELECTION`。
...
- `PromptTemplateDetailResponseDto`
- `PromptVersionListResponseDto`
- `CheckpointListResponseDto`
- `CheckpointDetailResponseDto`
- `CheckpointValidationResponseDto`
- `TriggerEvaluationResponseDto`
...
- `TaskDetailResponseDto`：若当前任务存在待处理的权限确认、高风险执行确认或人工介入处置，还需返回 `interactionDedupKey?`、`interactionDecisionStatus?`、`interactionResolvedAt?`、`factSummary?`；其中 `factSummary` 至少包含 `taskId`、`currentStatus`、`triggerReason`、`impactSummary`、`pendingActionLabel`，用于支持非原始接入端在处理同一事实前展示统一的最小事实摘要，而 `interactionDedupKey` / `interactionDecisionStatus` 用于表达该事实是否已被其他接入端先行处理。（来源：N-01.4，N-01.6，N-13，N-14）
...
- `TaskDetailResponseDto`：当任务已进入 Agent 执行链路时，还需返回 `currentReasoningSummary?`、`lastObservationSummary?`、`nextActionSummary?`、`reasoningUpdatedAt?`、`planRevision?`、`replanCount?`、`latestPlanSummary?`、`goalSatisfactionStatus?`、`goalSatisfactionSummary?`，用于表达当前回合的简短推理摘要、最近一次有效工具观察摘要、下一步动作意图，以及 Leader 是否已基于新观察结果完成目标满足性判断并进入下一轮增量重编排；这些字段只能承载可审计摘要，不得承载完整原始思维链。其中 `planRevision` / `replanCount` 用于稳定表达父任务在同一用户请求下的多轮编排事实，`goalSatisfactionStatus` 至少支持 `unknown`、`unsatisfied`、`satisfied`、`manual_intervention_required`。（来源：N-03.1，N-07.5，N-12，N-13，N-14）
...
- `TaskTimelineResponseDto`：当事件属于权限确认、确认执行、人工介入处置等一次生效型交互时，事件级或 `payload` 还需返回 `interactionDedupKey?`、`interactionDecisionStatus?`、`resolvedByClientId?`、`resolvedAt?`、`factSummary?`，用于支持多端并发交互去重、重复裁决拒绝和最终处理结果的实时同步。（来源：N-01.4，N-13，N-14）
- `TaskTimelineResponseDto`：父任务时间线还需支持 `SubTasksCreated`、`DomainTaskQueued`、`DomainTaskScheduled`、`DomainTaskTriggered`、`TaskArrangementCompleted`、`TaskEtaEstimated`、`GoalSatisfactionEvaluated`、`TaskReplanned` 等事件，用于表达子任务安排确认、ETA 聚合，以及 Leader 基于新观察结果执行目标满足性判断和增量重编排的过程。（来源：N-02.2，N-03.1，N-03.2，N-07.2，N-07.3，N-07.5，N-14）
...
- `TaskReportResponseDto`：还需支持 `reasoningTraceSummary?`、`observationTraceSummary?`、`reactLoopSummary?`、`replanTraceSummary?`，用于聚合展示本次任务中关键 ReAct 回合的推理摘要、工具观察摘要、“观察如何改变后续动作”的执行结论，以及 Leader 在同一父任务中进行了几轮目标满足性判断与增量重编排，供复盘与排障使用；这些字段必须是压缩后的结构化摘要，不得直接返回完整原始思维链。（来源：N-03.1，N-07.5，N-12，N-13，N-14）
...
- `TaskOutputsResponseDto`：当结果页需要解释 Agent 如何得出当前输出时，还需包含 `reasoningSummary?`、`observationSummary?` 或等价摘要字段，用于解释关键步骤摘要和关键工具观察，但不得以结果接口暴露完整原始思维链。（来源：N-04.1，N-05.1，N-07.5，N-12，N-14）
...
- `SkillListResponseDto` / `SkillDetailResponseDto`：除基础 Skill 信息外，还需返回 `runtimeLanguage`、`version`、`installStatus?`、`validationStatus?`、`validationSummary?`；其中 `runtimeLanguage` 用于表达 Skill 包声明的唯一运行时语言，`validationStatus` / `validationSummary` 用于说明是否通过单包单语言与运行时一致性校验。（来源：N-11）
- `SkillInstallLifecycleResponseDto`：用于 `POST /api/skills/:id/install`、`POST /api/skills/:id/uninstall`、`GET /api/skills/:id/status`；除 `skillId`、`success`、`message` 外，还需返回 `lifecycleStatus?`、`errorCode?` 与 `validation?`（包含 scripts/dependencies/executor 三维度）。（来源：N-11）
- `PromptTemplateListResponseDto` / `PromptTemplateDetailResponseDto`：除基础 Prompt 信息外，还需返回 `type`（leader/domain）、`currentVersion`、`versions[]`、`schema`、`createdAt`、`updatedAt`；其中 `versions[]` 需包含版本号、变更原因和创建时间。（来源：N-12）
- `AgentDetailResponseDto`：当返回 Agent 配置详情时，还需包含 `promptProfile`（`templateId`、`templateVersion`、`profileVersion`、`roleDefinition`、`behaviorRules`、`capabilityBoundaries`）与 `lastPromptMigration?`（迁移状态、冲突摘要、回退结果）。（来源：N-07.1，N-12，N-14）
- `TaskDetailResponseDto`：当任务进入执行链路后，还需返回 `promptTemplateId?`、`promptTemplateVersion?`、`agentPromptProfileVersion?`、`promptMigrationId?`，用于追溯本次执行使用的模板结构版本和 Agent 独立内容版本。（来源：N-12，N-14）
- `CheckpointListResponseDto` / `CheckpointDetailResponseDto`：包含 `checkpointId`、`taskId`、`stepId`、`createdAt`、`snapshot`（包含 taskInput、memoryScope、executedToolCalls、goalSatisfactionStatus 等）、`validation`（包含 integrity、idempotency、timeliness、validatedAt）；用于展示检查点列表和详情。（来源：N-14.2）
- `CheckpointValidationResponseDto`：包含 `checkpointId`、`validation`（完整性、幂等性、时效性验证结果）、`safetyProof`、`recoveryDecision`（auto_recover/manual_confirm/cannot_recover）、`reason`、`affectedSteps`；用于验证检查点是否可安全恢复。（来源：N-14.2）
- `TriggerEvaluationResponseDto`：包含 `taskId`、`triggerType`、`canRelease`、`evaluationSummary`、`nextCheckAt?`、`anomalyDetected?`、`anomalyCode?`、`anomalySummary?`、`suggestedActions?`；用于返回触发条件评估结果。（来源：N-01.2，N-14）
...
- `SKILL_IMPORT_FAILED`
- `SKILL_RUNTIME_LANGUAGE_INVALID`
...
- `CHECKPOINT_IDEMPOTENCY_FAILED`
- `CHECKPOINT_TIMELINESS_FAILED`
...
- `TRIGGER_RE_EVALUATION_FAILED`
- `WAIT_ANOMALY_DETECTED`
...
- 入口判定和会话主交互相关事件 `payload` 若承载一次生效型交互，还需包含 `interactionDedupKey?`、`interactionDecisionStatus?`、`resolvedByClientId?`、`resolvedAt?`、`factSummary?`，用于表达同一事实是否已被处理、由哪个接入端处理，以及用于界面渲染的最小事实摘要。
...
- Agent 执行与工具观察相关事件 `payload` 还需支持 `reasoningSummary?`、`nextActionSummary?`、`observationSummary?`、`observationSourceType?`、`observationRef?`、`usedObservationIds?`、`planRevision?`、`replanCount?`、`goalSatisfactionStatus?`、`goalSatisfactionSummary?`、`replanReason?`、`latestPlanSummary?`，用于表达 ReAct 回合中的简短推理摘要、下一步动作意图、工具或检索观察结果、被后续决策消费的观察引用，以及 Leader 基于这些观察对父任务目标是否满足所做的判断和后续增量重编排事实；上述字段只表达对外展示与审计所需摘要，不得承载完整原始思维链。
- 权限相关事件的 `payload` 需包含 `permissionMode`、`decisionSource`，在待权限确认场景下还需包含 `permissionRequestId` 与 `decisionTraceSummary`。
- 权限相关事件的 `payload` 在多端确认场景下还需明确区分 `permissionDecisionTraceSummary` 与 `interactionDecisionStatus`：前者用于表达 `deny > ask > allow` 的权限策略裁决依据，后者只用于表达同一事实是否已被其他接入端先行处理，二者不得混用为同一优先级语义。
...
- `POST /api/knowledge/upload`
  - 入参：multipart `file`，可选 `agentId`
  - 出参：`{ docId, title, overview, sourceType, chunkCount }`
  - 语义：上传文件后自动解析正文、生成标题与总览，并完成 LanceDB 入库

- `PATCH /api/knowledge/:id/title`
  - 入参：`{ title: string }`
  - 出参：`{ success: true }`
  - 语义：更新知识文档标题
...
- `POST /api/knowledge` 保留兼容，但知识库页面默认不再走手工正文创建路径。
