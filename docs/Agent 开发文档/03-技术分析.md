...
- **滑动窗口算法**：
  - 维护 `max_token_limit`（例如 4096）。
  - 新消息写入前计算 `current_total + new_message_token`，若超限则从最早消息开始删除或压缩为摘要，直至满足限制。
  - 引入“重要性评分”字段，优先保留包含实体、指令、约束的消息，优先删除纯寒暄类内容。
- **应用场景**：
