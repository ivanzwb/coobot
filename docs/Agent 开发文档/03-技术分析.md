...
本文档旨在明确系统的技术架构、核心组件选型、关键算法策略及非功能性保障措施。重点解决**本地优先架构下的资源约束**、**多智能体并发调度**、**知识库隔离检索**及**任务全链路可观测性**等技术难题，为后续概要设计与详细开发提供技术基线。
...
- **依赖服务**：
  - **LLM 后端**：支持本地或远程 API（OpenAI/OpenRouter）。
...
- 定义严格的状态流转逻辑，确保可追踪性：
  - **状态枚举**：WAITING_FOR_LEADER -> CLARIFICATION_PENDING -> PARSING -> DISPATCHING -> QUEUED -> RUNNING -> AGGREGATING -> COMPLETED / EXCEPTION / TERMINATED。
  - **触发方式**：通过 `trigger_mode` 字段区分（immediate/scheduled/event_triggered），定时任务使用 WAITING_FOR_LEADER + triggerMode='scheduled'。
...
- **适配器模式**：
  - OpenRouterAdapter: 处理 OpenRouter API 协议。
  - OpenAIAdapter: 处理标准 OpenAI API 协议。
...
- **实现 DENY > ASK > ALLOW 决策链**：
  ...
  - 高风险拦截：
    ...
    - 若策略为 ASK：挂起工具执行，经 WebSocket 发送 AUTH_REQUEST，会话页在消息流内展示授权卡片，等待用户 `auth/decision` 回调（带超时计时器）。
    - 若策略为 ALLOW：直接执行。
...
| 模型推理 | OpenAI 兼容 HTTP API | 本地和远程模型通过 OpenAI 兼容 API 访问。P0 建议优先对接 OpenAI |
