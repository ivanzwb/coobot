...
- `PATCH /api/conversations/:id`（来源：N-01.3，更新会话标题/元数据）
- `DELETE /api/conversations/:id`（来源：N-01.3，删除会话）
- `GET /api/conversations/:id/messages`（来源：N-01.1，N-01.4，N-05.1）
- `POST /api/conversations/:id/messages`（来源：N-01.1，发送消息）
...
- `POST /api/conversations/batch-delete`（来源：N-01.3，批量删除会话）
